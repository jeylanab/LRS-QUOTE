...
const Step3bGateAccess = ({ onSelect }) => {
  ...
  const handleContinueNoAccess = () => {
    onSelect(false); // continue with no rear access
  };
      ...
      <div className="animate-drop-in delay-0 flex flex-col gap-5 p-6 bg-white rounded-3xl border-2 border-sky-mid shadow-card">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-navy/60 leading-relaxed">
            No problem! Without side or rear gate access, we can offer you a <span className="font-black text-navy">front of house clean only</span> due to access issues getting to the rear of the property.
          </p>
          <p className="text-sm font-medium text-navy/60 leading-relaxed mt-3">
            Your quote will be adjusted accordingly and we'll confirm the details when we get in touch.
          </p>
        </div>

        <div className="flex flex-col gap-2 w-full">
          <button
            onClick={handleContinueNoAccess}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg w-full">
        ...
        >
          <div className="relative mb-4">
            <img src={gate1Img} alt="Yes via gate" className="h-28 w-auto object-contain drop-shadow-md group-hover:scale-105 transition-transform duration-200" />
          </div>
          <span className="font-black uppercase text-sm text-navy/70 group-hover:text-navy transition-colors">
            Yes, via gate
          </span>
          <span className="text-[10px] font-medium text-navy/40 mt-1 text-center">
            We can access front and rear
          </span>
        </button>
        ...
        >
          <div className="relative mb-4">
            <img src={gate0Img} alt="No access" className="h-28 w-auto object-contain drop-shadow-md group-hover:scale-105 transition-transform duration-200" />
          </div>
          <span className="font-black uppercase text-sm text-navy/70 group-hover:text-navy transition-colors">
            No Access
          </span>
          <span className="text-[10px] font-medium text-navy/40 mt-1 text-center">
            No side or rear gate
          </span>
        </button>
