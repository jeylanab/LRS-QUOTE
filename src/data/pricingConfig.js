...
export const pricingConfig = {
  ...
  windowCleaning: {
    'semi-detached': { '2-3': 20, '4': 24 },
    'detached':      { '2': 20, '3': 24, '4': 26 },
    // PLACEHOLDER — confirm with Liam
    'terraced':      { '1-2': 18, '3': 20, '4': 22 },
    'town-house':    { '2-3': 24, '4': 28, '5': 32 },
    'bungalow':      { '1-2': 18, '3': 20, '4': 24 },
    'flat':          { '1-2': 18, '3': 22, '4': 26 },
  },
...
export const getServiceTierKey = (propertyType, bedrooms) => {
  if (propertyType === 'semi-detached') return bedrooms === '4' ? '4-bed-semi' : '3-bed-semi';
  if (propertyType === 'detached') {
    if (bedrooms === '2') return '2-bed';
    if (bedrooms === '3') return '4-bed-semi';
    if (bedrooms === '4') return '4-bed-det';
  }
...
export const calculateWindowTotal = (data) => {
  const { propertyType, bedrooms, hasConservatory, hasExtension } = data;
  ...
  return base + conservatoryAddon + extensionAddon;
};
